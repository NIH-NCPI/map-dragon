...
export const OntologyCheckboxes = ({
  ...
  if (
    ...
    apiPreferencesCode.umls = selectedOntologies;
  }
  if (
    apiPreferencesCode &&
  ...
  // If the above are false, selectedOntologies are used for the search
  else {
  ...
  useEffect(() => {
    ...
    setCheckedOntologies(prev => {
      if (prev.length === onts.length && prev.every((v, i) => v === onts[i])) {
        return prev;
      }
      return onts;
    });
